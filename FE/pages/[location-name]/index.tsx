...
const LocationPage: NextPage<LocationPageProps> = ({ locationName, locations: initialLocations }) => {
  ...
  // const { 'location-name': locationName } = router.query; // We get this from props now
  ...
  // Add longPressRef and touchTimeoutRef
  const longPressRef = useRef(false);
  const touchTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Replace with lastTapTimestamp ref for double tap detection
  ...
  // Debug effect to see when locationName changes
  useEffect(() => {
    console.log('Location Name changed:', locationName);
    console.log('Router Query:', router.query);
    console.log('Router Pathname:', router.pathname);
  }, [locationName, router.query, router.pathname]);
  ...
  return (
    ...
    >
      ...
      <Head key={Array.isArray(locationName) ? locationName[0] || 'default' : locationName || 'default'}>
        ...
        <meta
          name="description"
          content="Lafa's List is a comprehensive list of events, venues, and artists in your city. Find the best events, venues, and artists in your city."
        />
        <link rel="icon" href="/favicon.ico" />
        
        {/* Always render meta tags, but with conditional content */}
        <link rel="canonical" href={locationName ? `https://lafaslist.com/${locationName}` : 'https://lafaslist.com'} />
        
        {/* Open Graph Meta Tags */}
        <meta property="og:title" content={locationName ? `lafaslist | ${formattedLocationName}` : "lafaslist"} />
        <meta property="og:description" content="Lafa's List is a comprehensive list of events, venues, and artists in your city. Find the best events, venues, and artists in your city." />
        ...
        <meta property="og:url" content={locationName ? `https://lafaslist.com/${locationName}` : 'https://lafaslist.com'} />
        ...
        <meta name="twitter:description" content="Lafa's List is a comprehensive list of events, venues, and artists in your city. Find the best events, venues, and artists in your city." />
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `
              ...
              console.log('Page URL: ${locationName ? `https://lafaslist.com/${locationName}` : 'https://lafaslist.com'}');
      ...
      <nav
        className={`fixed top-0 left-0 w-[100vw] flex md:flex-row flex-wrap flex-col gap-4 justify-between py-4 border-0 z-10 bg-midnight transition-transform duration-300 ex:gap-6 ex:mx-10 ex:ml-0 ex:pl-10 ex:py-6 ex:px-0 ex:pr-[56px] ${
        ...
        <div className="flex flex-1 justify-between md:justify-end gap-4 z-[2] pl-6 pr-0 max-w-[94vw] md:max-w-[calc(100vw-400px)] md:pr-6 ex:relative ex:px-0 ex:pr-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={
                'ex:flex ml-2 hidden group items-center gap-2 hover:bg-mist-white border-solid border-[1px] px-3 py-1 rounded-lg relative overflow-clip text-mist-white border-slate-black bg-transparent-white' +
                (sort.value
            ...
            {activeDropdown === 'sort' ? (
              <SortOverlay
                ...
                options={Constants.eventsSortingOptionsDashboard}
                isHidden={!isNavbarVisible}
              />
            ) : null}
            ...
            {(() => {
              ...
              sections.push(
                <EventsSection
                  ...
                  subTitle="Happening after next week"
                  events={getSiftedEvents(
                    sortedEvents,
                    'afterNextWeekEvents',
                    appliedFilters
          ...
          >
            ...
            >
              <p className="font-semibold text-lg">
                <span className="underline font-semibold">Sign up</span> to save your own list, it's free!
              </p>
            </div>
            <button 
              onClick={() => {
